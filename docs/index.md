...
為了省去逐項手動設定，我們準備了一份 `hermes-config.json`（包含 toolset、後端、預設模型等）。不必從零開始：直接[下載這份設定檔](https://drive.google.com/file/d/12yIhmfL0eAXm_MF9s8y4TwpL5I6mN_HF/view?usp=sharing)，在 Hermes Desktop 用「匯入設定」載入即可。

!!! note "關於模型"
    這份設定檔的預設模型是 `nvidia/nemotron-3-super-120b-a12b:free`，它本來就在模型選單裡，所以匯入後你也可以直接從下拉選單選。只有要換成選單裡沒有的模型時，才需要手動改設定檔——做法見後面的「選擇模型」一節。

## 申請 OpenRouter API 金鑰

要讓 Hermes 連上 OpenRouter，得先有一把 API 金鑰：
...
建好之後把金鑰複製起來，貼到 Hermes 的 API 設定裡。金鑰只會完整顯示一次，記得先存好。
...
1. 點「匯出設定」，打開 `hermes-config.json`。
