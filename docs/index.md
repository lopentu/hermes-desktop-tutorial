...
Hermes Desktop 的設定可以匯出成一份 `hermes-config.json` 來檢視或修改。請先用「匯出設定」匯出目前的設定，打開後找到 `terminal` 裡的 `backend` 欄位確認一下：
...
如果 `backend` 是 `"docker"`，請改成 `"local"`（原因見上面的「Backend：Docker vs. Local」），存檔後再用「匯入設定」載入。
