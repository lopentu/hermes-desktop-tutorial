...
完整的 `hermes-config.json` 設定很多（`providers`、`agent`、`terminal` 等），但今天你只需要注意最上面的 `model` 欄位——它決定 agent 用哪個模型：
...
不必從零開始：可以直接[下載這份設定檔](https://drive.google.com/file/d/1qq405zgppJGRtTIPJ3e6iGeQUI-kzjxi/view?usp=sharing)，在 Hermes Desktop 用「匯入設定」載入。
...
其他免費模型不會出現在下拉選單裡，所以要用「改設定檔」的方式換：
