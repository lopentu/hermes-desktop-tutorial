...
為了省去逐項手動設定，我們準備了一份 `hermes-config.json`（包含 toolset、後端、預設模型等）。不必從零開始：直接[下載這份設定檔](https://drive.google.com/file/d/1qq405zgppJGRtTIPJ3e6iGeQUI-kzjxi/view?usp=sharing)，在 Hermes Desktop 用「匯入設定」載入即可。
