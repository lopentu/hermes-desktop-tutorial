...
- **結果一致**：agent 每次處理同類任務的方式相同，不會因為 prompt 寫法不同而差異很大。
- **可預測、可重現**：固定的流程讓輸出更穩定，也更容易除錯和持續改進。
...
!!! warning "安裝技能前先確認來源"
    ...
    - **供應鏈攻擊**：有人會用相似名稱的假技能（typosquatting）混入目錄。
    - **程式碼執行**：技能內附的腳本有存取本機檔案的能力。

    安裝前可以到 skills.sh 的技能頁面，確認右側是否有安全審計結果（如 Gen Agent Trust Hub、Socket、Snyk），通過審計的技能風險相對較低。

    ![skills.sh 技能頁面右側顯示安全審計結果，三項均為 PASS](images/ask-questions.png){ width="700" }
